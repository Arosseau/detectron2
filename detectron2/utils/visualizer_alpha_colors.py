...
class Visualizer:
    def __init__(self, img_rgb, metadata, scale=1.0, instance_mode=ColorMode.SEGMENTATION):
        ...
        """
        img = np.asarray(img_rgb).clip(0, 255)
        self.img = np.zeros(img.shape).astype(np.uint8)
    ...
    def draw_instance_predictions(self, predictions):
        ...
        for i, mask in enumerate(masks):
            if int(classes_array[i]) in [0, 24]:
                filtered_masks.append(GenericMask(mask, self.output.height, self.output.width))
                if int(classes_array[i]) in [0]:
                    color = green
                elif int(classes_array[i]) in [24]:
                    color = red
                colors.append(color)
